...
try:
    ...
    ima_podadora = pg.image.load(os.path.join('assets', 'imagenes', 'podadora.webp'))

    #----GAME-OVER----
    ima_game_over = pg.image.load(os.path.join('assets', 'imagenes', 'game_over.jpg'))
    #ima_game_over = pg.image.load(os.path.join('assets', 'imagenes', 'game_over.png'))
    w, h = ima_patio.get_size()
    ima_game_over = pg.transform.scale(pg.image.load(os.path.join('assets', 'imagenes', 'game_over.jpg')), (w,h))
    #ima_game_over = pg.transform.scale(pg.image.load(os.path.join('assets', 'imagenes', 'game_over.png')), (w,h))
...
podadoras = []
for i in range(fil):
    #ponemos podadora por fila
    podadora = Podadora(i, margen_y, margen_x, c_size, ima_podadora)
    podadoras.append(podadora)
...
y_actual_boton = boton_pala_armado(botones_plantas, y_actual_boton, ima_pala_boton)
...
#---------SELECCIÓN PLANTAS---------
            ...
            for boton in botones_plantas:
                if boton.clic_en(event.pos): #.clic_en es nuestra funcion
                    ...
                    if isinstance(boton, BotonPlanta):
                        if pala_activada:
                            #deselecciona pala si ya activada (extra seguridad)
                            pala_activada = False 
                        if boton.esta_disponible() and soles_actuales >= boton.costo:
                        ...
                        if not (boton.esta_disponible() and soles_actuales >= boton.costo):
                            planta_seleccionada = None
                            planta_costo_seleccionada = 0
                            imagen_seleccionada_para_colocar = None
                    #'''
                    elif isinstance(boton, BotonPala):
                        # Deseleccionar planta si estaba seleccionada
                        planta_seleccionada = None
                        planta_costo_seleccionada = 0
                        imagen_seleccionada_para_colocar = None
                        pala_activada = not pala_activada # Activar/desactivar pala
                        print(f"Pala activada: {pala_activada}")
                    break
            ...
            if not clic_en_boton:
                ...
                if not sol_recolectado:
                    ...
                    if planta_seleccionada and margen_x <= mouse_x < (margen_x + ancho_grilla) and margen_y <= mouse_y < (margen_y + alto_grilla):
                        ...
                        if grilla_ocupada[celda_fila][celda_col] is None:
                                ...
                                if soles_actuales >= nueva_planta.costo:
                                    ...
                                    #Cooldown para la selección de plantas
                                    for boton in botones_plantas:
                                        if isinstance(boton, BotonPlanta) and boton.tipo_planta == planta_seleccionada:
                                            boton.iniciar_cooldown()
                                            break
                                    planta_seleccionada = None
                                    planta_costo_seleccionada = 0
                                    imagen_seleccionada_para_colocar = None
                    ...
                    elif pala_activada and margen_x <= mouse_x < (margen_x + ancho_grilla) and margen_y <= mouse_y < (margen_y + alto_grilla):
                        celda_col = int((mouse_x - margen_x) // c_size)
                        celda_fila = int((mouse_y - margen_y) // c_size)
                        if grilla_ocupada[celda_fila][celda_col] is not None:
                            planta_a_eliminar = grilla_ocupada[celda_fila][celda_col]
                            soles_actuales += planta_a_eliminar.costo // 2 # Reembolso
                            #El reembolso es una prop del pvz2, que viene bien para mantener el ritmo
                            plantas_en_juego.remove(planta_a_eliminar)
                            grilla_ocupada[celda_fila][celda_col] = None
                            print(f"Planta en Fila {celda_fila}, Columna {celda_col} eliminada.")
                            pala_activada = False # Desactivar pala después de usarla
                        else:
                            print("No hay planta en esta celda para eliminar.")
    ...
    for i, zombie in enumerate(zombis_activos):
        ...
        if resultado == 'gameOver':
            ...
            if not fila_destruida: 
                ...
                for podadora in podadoras:
                    if podadora.fila == zombie.fila:
...
#---------SCREEN---------
    ...
    # Capa 2: Grilla del juego y podadoras
    ...
    dibujar_barra_lateral(botones_plantas, soles_actuales, screen, planta_seleccionada if not pala_activada else "pala_activa", test_font)
    ...
    if pala_activada:
        mouse_pos = pg.mouse.get_pos()
        fantasma_pala = pg.transform.scale(ima_pala_boton, (c_size, c_size))
        fantasma_pala.set_alpha(150) # Semi-transparente
        screen.blit(fantasma_pala, (mouse_x - c_size // 2, mouse_y - c_size // 2))

    pg.display.update()
    clock.tick(FPS) # Control de velocidad, mantiene 60 FPS
